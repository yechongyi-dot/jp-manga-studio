...
/**
 * JpBroadcastBg — 「财经速报·暖调」专属动态金融背景（浅色）
 * 同套动态金融图表（蜡烛K线 / MA / BB / 成交量 / MACD / 需給動向），
 * 视觉：暖米白渐变 + 金橙网格 + 清晰红蓝蜡烛。温暖财经速报感，与 Prime 冷调区分。
 */
...
const MA5       = '#D9810A';
const MA25      = '#B45309';
const GRID      = 'rgba(150,100,40,0.05)';
const GRID_H    = 'rgba(150,100,40,0.10)';
const TXT       = 'rgba(96,66,34,0.56)';
const TXT_HI    = 'rgba(58,40,20,0.92)';
const PANEL_LN  = 'rgba(150,100,40,0.14)';
...
export const JpBroadcastBg: React.FC<Props> = ({ frame, trend = 'up', overlay = 0, seed }) => {
  const data = useMemo(() => {
  ...
  return (
    <div style={{
      ...
      background: 'radial-gradient(125% 85% at 50% 12%, #FFFDF9 0%, #FBF4E9 52%, #F4E9D6 100%)',
      ...
      <svg style={{ position: 'absolute', inset: 0 }} width={W} height={1920}>
        ...
        <rect x={0} y={KLINE_Y - 8} width={W} height={1920 - KLINE_Y} fill="url(#kbGrid)" />
        <rect x={0} y={scanY} width={W} height={2} fill="rgba(200,140,40,0.05)" />
      </svg>
      ...
      <svg style={{ position: 'absolute', top: KLINE_Y, left: 0 }} width={W} height={KLINE_H}>
        ...
        {(() => {
          ...
          return <polygon points={[...pts, ...lo].map(([x,y]) => `${x},${y}`).join(' ')} fill="rgba(180,120,40,0.045)" />;
        })()}
        {(['up','lo'] as const).map(band => {
          ...
          return pts.length > 1
            ? <polyline key={band} points={polyln(pts)} fill="none" stroke="rgba(180,120,40,0.24)" strokeWidth={1} strokeDasharray="5,5" />
            : null;
        })}
          ...
          return (
            <g key={i}>
              ...
              <rect x={x} y={f.corp >= 0 ? FLOW_H / 2 - hi : FLOW_H / 2}
                width={BODY_W * 0.44} height={Math.max(1, hi)} fill={f.corp >= 0 ? 'rgba(217,129,10,0.55)' : 'rgba(160,82,45,0.5)'} rx={1} />
            </g>
          );
        })}
        <circle cx={W - 268} cy={13} r={6} fill={UP_DIM} />
        <text x={W - 256} y={19} fill={TXT} fontSize={19} fontFamily={MONO}>外国人</text>
        <circle cx={W - 168} cy={13} r={6} fill="rgba(217,129,10,0.70)" />
      ...
      <div style={{ position: 'absolute', top: BOT_Y, left: 0, width: W, height: BOT_H, padding: '16px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ display: 'flex', gap: 26, fontSize: 30, fontWeight: 700, fontFamily: MONO }}>
          ...
          <span style={{ color: 'rgba(96,66,34,0.42)' }}>― 変わらず {mkt.flat.toLocaleString('ja-JP')}</span>
        ...
        <div style={{ display: 'flex', height: 7, borderRadius: 4, overflow: 'hidden', width: W - 44 }}>
          ...
          <div style={{ flex: mkt.flat, background: 'rgba(150,100,40,0.16)' }} />
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 2 }}>
          ...
          <span style={{ color: 'rgba(150,100,40,0.45)', fontSize: 22, fontFamily: MONO }}>KST {timeStr}</span>
        </div>
      </div>

      {overlay > 0 && <div style={{ position: 'absolute', inset: 0, background: `rgba(255,253,248,${overlay})`, pointerEvents: 'none' }} />}
    </div>
  );
};
